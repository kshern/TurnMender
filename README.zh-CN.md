...
- 可设置同一任务连续自动继续的次数上限（1–100 次，默认 10 次）。
- 可自定义发送到原任务的继续指令，并随时恢复默认内容。
- 使用彩色托盘图标提示正常、需要留意和需要处理三种状态。
- 主窗口关闭后继续在托盘运行；托盘菜单可查看状态、切换自动继续、直达待处理任务、打开设置与日志，或彻底退出。
...
当前实现会识别包含以下英文原文的消息（不区分大小写）：

- `Selected model is at capacity`
- `Error running remote compact task: Selected model is at capacity`

此外，当错误信息字段为 `server_overloaded` 时，也会判定为模型容量不足。仅识别到容量错误还不会立即发送消息，程序仍会继续检查 `last_agent_message` 和其他安全条件。
...
- 同一任务连续自动继续达到设置的次数上限后转为人工处理；上限可在设置中配置为 1–100 次，默认为 10 次。
...
托盘菜单顶部会说明当前状态，并提供自动继续勾选开关。存在待处理任务时，菜单最多列出最近 3 个，点击即可打开对应 Codex 任务；更多任务可回到主窗口查看。菜单还可直接打开设置和运行日志。顶部保留自动继续快速开关；连续次数上限、继续指令和界面语言集中在设置中。关闭主窗口只会隐藏窗口；如需完全退出，请使用托盘菜单中的“退出 TurnMender”。
...
目录中的 `state.json` 保存去重与连续计数，`config.json` 保存自动继续开关、次数上限和自定义继续指令，`turnmender.log` 保存运行日志。

所有判断都在本机完成。状态文件不保存任务正文；日志只记录任务 ID、轮次 ID、关键自动继续结果和异常。正常监听状态以主界面和托盘为准，日志不会再周期性写入心跳记录。运行日志超过 5 MB 后会自动换成新文件，并保留一份 `.1` 结尾的旧日志，避免长期运行时无限增长。
